...
public class RobotSumoAgentWL : RobotAgent
{
    public bool IsDebug;
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        else if (arrayFuzzy[3] < _maxRange/2 && arrayFuzzy[4] < _maxRange/2 && motors[0].powerMotor > 15f &&
                 motors[1].powerMotor > 20f)
        {
            tmp = 4;
            SetReward(0.2f);
        }
        else if (arrayFuzzy[2] < _maxRange/2 && arrayFuzzy[3] < _maxRange/2 && motors[0].powerMotor > 20f &&
                 motors[1].powerMotor > 15f)
        {
            tmp = 5;
            SetReward(0.2f);
        }
        else if (arrayFuzzy[2] < _maxRange/2 && arrayFuzzy[3] < _maxRange/2 && arrayFuzzy[4] < _maxRange/2 && motors[0].powerMotor > 25f &&
                 motors[1].powerMotor > 25f)
        {
            tmp = 6;
            SetReward(0.5f);
        }
